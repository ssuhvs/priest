...
JAVA_JMX_OPTS=""
if [ "$1" = "jmx" ]; then
    JAVA_JMX_OPTS=" -Dcom.sun.management.jmxremote.port=$2 -Dcom.sun.management.jmxremote.ssl=false \
    -Dcom.sun.management.jmxremote.authenticate=false -Djava.rmi.server.hostname=127.0.0.1 -Dcom.sun.management.jmxremote "
fi
JAVA_MEM_OPTS=""
...
while [ $COUNT -lt 1 ]; do    
    ...
    #else
    	COUNT=`ps  -ef | grep -v grep | grep java | grep "$DEPLOY_DIR" | awk '{print $2}' | wc -l`
    #fi
	if [ $COUNT -gt 0 ]; then
		break
	fi
...
echo "OK!"
PIDS=`ps  -ef | grep -v grep | grep java | grep "$DEPLOY_DIR" | awk '{print $2}'`
echo "PID: $PIDS"
